...
function sss(data){
    ...
    $(".user-info").removeClass("choosed");
    $('<div class="banker">').appendTo('.user-info[data-pos="' + indexuser + '"] .user-img');
    $(".user-info[data-pos=" + indexuser + "]").addClass("choosed");
    $('.rob-zhuang-num').empty();
}
...
function operationButton(data){
    ...
    if(data=='7'){
        // 摊牌
        html+='<div class="showdown" style="display: block;" onclick="tanpaime();"></div>';
    }
...
function Gold(source, target) {
    ...
    function move() {
        ...
        if (obj[0][0].index == index) {
            if (id_bol) {
                $('.user-info[data-pos="' + target + '"]').addClass('flash');
            } else {
                for (var i = 0; i < target.length; i++) {
                    $('.user-info[data-pos="' + target[i] + '"]').addClass('flash');
                }
            }
        } else if (obj[0][count - 1].index == index) {
            if (id_bol) {
                $('.user-info[data-pos="' + target + '"]').removeClass('flash');
            } else {
                for (var i = 0; i < target.length; i++) {
                    $('.user-info[data-pos="' + target[i] + '"]').removeClass('flash');
                }
            }
        }
        _index++;
