...
function appCommand(data) {
    ...
    if (data.game == 1) {
        gameName = '六人牛';
        gameLinkName = 'sixoxen';
    } else if (data.game == 2) {
        gameLinkName = 'goldFlower';
        gameName = '金花';
        if (data.room_rule.chip_rule == 1) {
            card_rule = "分数:2/4，4/8，8/16，10/20";
        } else if (data.room_rule.chip_rule == 2) {
            card_rule = "分数:2/4，5/10，10/20，20/40";
        }
        upper_limit = '上限:' + data.room_rule.upper_limit;
        appContent = '规则：' + data.count_matchs + '局 ' + '底分' + data.room_rule.end_points + '分 ' + (upper_limit.length > 0 ? upper_limit + ' ': '') + (card_rule.length > 0 ? card_rule + ' ': '');
    } else if (data.game == 3) {
        gameName = '十三水';
        gameLinkName = 'thirteen';
        if (data.room_rule.play_type == 1) {
            play_type = '经典';
        }
        appContent = '规则：' + data.count_matchs + '局 ' + (play_type.length > 0 ? play_type + ' ': '') + '底分' + data.room_rule.end_points + '分 ';
    } else if (data.game == 4) {
        gameName = '十人牛';
        gameLinkName = 'tenoxen';
    } else if (data.game == 5) {
        gameName = '德州扑克';
        gameLinkName = 'texasHoldem';
        if (data.room_rule.end_points == 1) {
            mang = '1/2'
        } else if (data.room_rule.end_points == 2) {
            mang = '2/4'
        }
        if (data.room_rule.end_points_rule == 0) {
            end_points_rule = '无'
        } else if (data.room_rule.end_points_rule == 1) {
            end_points_rule = '1倍小盲'
        } else if (data.room_rule.end_points_rule == 2) {
            end_points_rule = '2倍小盲'
        }
        appContent = '规则：' + data.count_matchs + '局 小盲/大盲:' + mang + ' 前注:' + end_points_rule + ' 初始分数:' + data.room_rule.init_points;
    } else if (data.game == 6) {
        gameName = '十人三公';
        gameLinkName = 'threeOpens';
        if (data.type == 1) {
            typeText = "抢庄模式";
        } else if (data.type == 2) {
            typeText = "通比模式";
        ...
        if (data.room_rule.card_rule == 1) {
            card_rule = "";
        } else if (data.room_rule.card_rule == 2) {
            card_rule = "暴玖×9";
        }
        appContent = '规则：' + data.count_matchs + '局 ' + (typeText.length > 0 ? typeText + '; ': '') + '底分' + data.room_rule.end_points + '分 ' + (card_rule.length > 0 ? card_rule + ' ': '');
    } else if (data.game == 7) {
        gameName = '填大坑';
        gameLinkName = 'fillingPit';
        if (data.room_rule.games_mode == 1) {
            games_mode = '半坑（满5人10起）';
        } else if (data.room_rule.games_mode == 2) {
            games_mode = '半坑（满5人9起）';
        } else if (data.room_rule.games_mode == 3) {
            games_mode = '半坑（满4人J起）';
        } else if (data.room_rule.games_mode == 4) {
            games_mode = '全坑（2-A）';
        }
        var playType = data.room_rule.play_type;
        for (num in playType) {
            if (playType[num] == 1) {
                shareValueText += '带王 ';
            } else if (playType[num] == 2) {
                shareValueText += '王中炮 ';
            }
        }
        appContent = '规则：' + data.count_matchs + '局 ' + (games_mode.length > 0 ? games_mode: '') + (data.room_rule.sorce_type == 1 ? '烂锅翻倍': '') + ' ' + '底注' + data.room_rule.end_points + '分 喜分' + data.room_rule.happy_points + '分 ' + (shareValueText.length > 0 ? shareValueText + ' ': '');
    } else if (data.game == 8) {
        gameName = '六人牛';
        gameLinkName = 'bull6';
    } else if (data.game == 9) {
        gameName = '九人牛';
        gameLinkName = 'bull9';
    } else if (data.game == 10) {
        gameName = '十二人牛';
        gameLinkName = 'twelveoxenV2';
    } else if (data.game == 11) {
        gameName = '十三人牛';
        gameLinkName = 'thirteenoxen';
    } else if (data.game == 12) {
        gameName = '六人三公';
        gameLinkName = 'threeOpens6';
    } else if (data.game == 13) {
        gameName = '九人三公';
        gameLinkName = 'threeOpens9';
    }
    if (data.game == 1 || data.game == 4 || data.game == 11) {
        if (data.type == 1) {
            typeText = "明牌抢庄";
        } else if (data.type == 2) {
            typeText = "通比牛.牛";
            ...
            typeText = "固定庄家";
            zhuangValue = '庄家底分:' + data.room_rule.zhuang_value;
        }
        if (data.room_rule.card_rule == 1) {
            card_rule = "牛.牛×3 牛九×2 牛八×2";
        } else if (data.room_rule.card_rule == 2) {
            card_rule = "牛.牛×4 牛九×3 牛八×2 牛七×2";
        }
        var arrValue = data.room_rule.hand_patterns;
        for (num in arrValue) {
            if (arrValue[num] == 1) {
                shareValueText += '五花牛（5倍）';
            } else if (arrValue[num] == 2) {
                shareValueText += '炸弹牛（6倍）';
            } else if (arrValue[num] == 3) {
                shareValueText += '五小牛（8倍）';
            }
        }
        appContent = '规则：' + data.count_matchs + '局 ' + (typeText.length > 0 ? typeText + ' ': '') + '底分' + data.room_rule.end_points + '分 ' + (card_rule.length > 0 ? card_rule + ' ': '') + (shareValueText.length > 0 ? shareValueText + ' ': '') + (zhuangValue.length > 0 ? zhuangValue + ' ': '');
    }
    appTitle = WEB_TITLE + '：' + gameName;
    appPassword = '房间ID：' + data.number;
    appLink = '房间链接：http://' + DOMAIN + '/' + gameLinkName + '.html?code=' + data.code;
    var shareText = appTitle + '\n\r' + appContent + '\n\r' + appPassword + '\n\r' + appLink;
    var code = '<div class="app-share-masks">';
    code += '<div class="app-share-join">';
    code += '<div class="content-text">' + gameName + '(' + data.number + ')创建完成！</div>';
    code += '<div class="join-room">加入房间</div><p>&nbsp;</p>';
    code += '<div class="share-btn">分享房间</div>';
    code += '<div class="close-btn"></div>';
    code += '</div>';
    code += '</div>';
    $(code).appendTo('.index');
    $('.app-share-masks .join-room').click(function() {
        app_command('createroom', {
            'page': gameLinkName,
            'code': data.code
        });
    });
    $('.app-share-masks .share-btn').click(function() {
        app_command('share', {
                'content': shareText
            },
            function() {
                $('.app-share-masks').remove();
            });
    });
    $('.app-share-masks .close-btn').click(function() {
        playType = '';
        arrValue = '';
        gameName = '';
        gameLinkName = '';
...
var Page = {
    ...
    moreFunc: function() {
        var code = '<div class="more-mask2"><div class="content-mask"><div class="content-box"><i class="mask-icon mask-top"></i><i class="mask-icon mask-right"></i><i class="mask-icon mask-bottom"></i><i class="mask-icon mask-left"></i><div class="content-item"><div class="item switch">切换平台</div></div><div class="content-item"><div class="item user-friend">好友管理</div></div><div class="content-item"><div class="item pk-history">历史战绩</div></div><!--<div class="content-item"><div class="item user-update">更新资料</div></div>--><div class="content-item"><div class="item more-room-card">房卡明细</div></div><div class="content-item"><div class="item recoverCards">找回账户</div></div><div class="content-item"><div class="item log-out">注销登录</div></div><div class="content-item last"><div class="item error">异常举报</div></div><div class="close-more"></div></div></div></div>';
        $(code).appendTo('.index');
    },
    picUpload: function() {
    ...
    load: function() {
        if (typeof Page.template == 'function' && win.app > 0) {
            Page.template();
        }
        notice.start();
        $('.issue-room-card').click(function() {
            if (win.app == 1) {
                location.href = 'createGift.html?app=1';
            } else if (win.app == 2) {
                location.href = 'createGift.html?app=2';
            } else {
                location.href = 'createGift.html';
            }
        });
        $('.index .top .room-card-record').click(function() {
            if (typeof Page.moreFuncMask == 'function') {
                Page.moreFuncMask();
            } else {
                Page.moreFunc();
            }
            $('.index .more-mask2 .close-more , .index .more-mask2').click(function() {
                $('.index .more-mask2').remove();
            });
            $('.index .more-mask2 .recoverCards').click(function() {
                location.href = 'recoverCards.html';
                $('.index .more-mask2').remove();
            });
            $('.index .more-mask2 .user-friend').click(function() {
                if (win.app == 1) {
                    location.href = 'permissions.html?app=1';
                } else if (win.app == 2) {
                    location.href = 'permissions.html?app=2';
                } else {
            ...
            $('.index .more-mask2 .pk-history').click(function() {
                location.href = 'pkquery.html';
                $('.index .more-mask2').remove();
            });
            $('.index .more-mask2 .switch').click(function() {
                if (win.app > 0) {
                    app_command('changeplatform');
                    $('.index .more-mask2').remove();
                } else {
                ...
                $('.index .picture-box .submit').click(function() {
                    if (Page.time == 0) {
                        ...
                        if (!Page.picText) {
                            ...
                            ajax('home/user/postFeedback', {
                                    'group_path': Page.picData,
                                    'content': Page.picText
                                },
                                function(d) {
                                    if (d.info) {
                                        $.alert(d.info,
        ...
        Page.fishPrawnCrabGame();
        Page.pushBobbinGame();
        $('.fishPrawnCrab-mask .createRoomBtn').on('click',
            function() {
                if (Page.time == 0) {
                    Page.time = 2;
                    var index = setInterval(function() {
                        ...
                        1000);
                    Page.fishPrawnCrabSubmit();
                }
            });
        $('.pushBobbin-mask .createRoomBtn').on('click',
            function() {
                if (Page.time == 0) {
                    Page.time = 2;
                    var index = setInterval(function() {
        ...
        $('.fillingPitcard-mask .createRoomBtn').on('click',
            function() {
                if (Page.time == 0) {
                    Page.time = 2;
                    var index = setInterval(function() {
        ...
        Page.pushBobbinSwitch('.pushBobbin-mask');
    ...
    fishPrawnCrabGame: function() {
        $('.game-list .fishPrawnCrab-bg').one('click',
            function() {
                var data = storage.get('roomrule_fishPrawnCrab');
                if (data) {
                    $('.fishPrawnCrab-mask .tab span[data-item="' + data.zhuang_type + '"]').click();
                    $('.fishPrawnCrab-mask [name="fishPrawnCrab_chip_value"][value="' + data.chip_value + '"]').click();
                    $('.fishPrawnCrab-mask [name="fishPrawnCrab_zhuang_value"][value="' + data.zhuang_value + '"]').click();
                    $('.fishPrawnCrab-mask [name="fishPrawnCrab_max_matches"][value="' + data.max_matches + '"]').click();
                }
                $('.fishPrawnCrab-mask').show();
                $('.game-list .fishPrawnCrab-bg').on('click',
                    function() {
                        $('.fishPrawnCrab-mask').show();
                    });
            });
        $('.fishPrawnCrab-mask .content-box .close-window').click(function() {
            $('.fishPrawnCrab-mask').hide();
        });
    },
    fishPrawnCrabSubmit: function() {
        var zhuang_type = $('.fishPrawnCrab-mask .tab span.tab-item.on').attr('data-item');
        var chip_value = $('.fishPrawnCrab-mask [name="fishPrawnCrab_chip_value"]:checked').val();
        var zhuang_value = $('.fishPrawnCrab-mask [name="fishPrawnCrab_zhuang_value"]:checked').val();
        var max_matches = $('.fishPrawnCrab-mask [name="fishPrawnCrab_max_matches"]:checked').val();
        var data = {
        ...
        storage.set('roomrule_fishPrawnCrab', data);
        ajax('home/gameFishPrawnCrab/createRoom', data,
            function(d) {
                if (d.status == 1) {
                    $('.fishPrawnCrab-mask').hide();
                    if (USE_QRCODE) {
                        var url = 'http://' + JUMP_DOMAIN + '/fishPrawnCrab.html?code=' + d.info.code;
                        qrcodeCreate(url, 15, generalQrcodeData(d.info));
                        if (parseInt(d.info.count_matchs) === 12) {
                            $('.user-info .room-card span').text(Page.roomCard - 3);
                            Page.roomCard = parseInt($('.user-info .room-card span').text());
                        } else if (parseInt(d.info.count_matchs) === 24) {
                            $('.user-info .room-card span').text(Page.roomCard - 6);
                            Page.roomCard = parseInt($('.user-info .room-card span').text());
                        }
                    } else {
                        location.href = 'fishPrawnCrab.html?code=' + d.info.code;
                    }
                } else if (d.status == 2) {
                    $.alert('已有创建房间', 'error');
                } else if (d.status == 3) {
                    Page.phoneMask('.fishPrawnCrab-mask');
                } else {
                    $.alert(d.info);
                }
            });
    },
    pushBobbinGame: function() {
        $('.game-list .pushBobbin-bg').one('click',
            function() {
                var data = storage.get('roomrule_pushBobbin');
                if (data) {
                    $('.pushBobbin-mask .tab span[data-item="' + data.zhuang_type + '"]').touch();
                    if (data.zhuang_type == 2) {
                        $('.pushBobbin-mask [name="push_bobbin_shang_zhuang"][value="' + data.shange_zhuang + '"]').click();
                    } else if (data.zhuang_type == 1) {
                        $('.pushBobbin-mask [name="push_bobbin_chouma"][value="' + data.chouma + '"]').click();
                    }
                    $('.pushBobbin-mask [name="push_bobbin_rule"][value="' + data.card_rule + '"]').click();
                    $('.pushBobbin-mask [name="push_bobbin_max_matches"][value="' + data.max_matches + '"]').click();
                }
                $('.pushBobbin-mask').show();
                $('.game-list .pushBobbin-bg ').on('click',
                    function() {
                        $('.pushBobbin-mask').show();
                    });
            });
        $('.pushBobbin-mask .content-box .close-window').click(function() {
            $('.pushBobbin-mask').hide();
        });
    },
    pushBobbinSubmit: function() {
        var zhuang_type = $('.pushBobbin-mask .tab span.tab-item.on').attr('data-item');
        var value = $('[name=' + ('push_bobbin_' + $('.pushBobbin-mask .tab-item.on').attr('_class')) + ']:checked').val();
        var card_rule = $('[name="push_bobbin_rule"]:checked').val();
        var max_matches = $('[name="push_bobbin_max_matches"]:checked').val();
        var data = {
            'zhuang_type': zhuang_type,
            'max_point': card_rule,
            'max_matches': max_matches
        ...
        if (zhuang_type == 2) {
            data['shange_zhuang'] = value;
        } else if (zhuang_type == 1) {
            data['chouma'] = value;
        }
        storage.set('roomrule_pushBobbin', data);
        ajax('home/gameErbagang/createRoom', data,
            function(d) {
                if (d.status == 1) {
                    $('.pushBobbin-mask').hide();
                    if (USE_QRCODE) {
                        var url = 'http://' + JUMP_DOMAIN + '/pushBobbin.html?code=' + d.info.code;
                        ...
                        location.href = 'pushBobbin.html?code=' + d.info.code;
                    }
                } else if (d.status == 2) {
                    $.alert('已有创建房间', 'error');
                } else if (d.status == 3) {
                    Page.phoneMask('.pushBobbin-mask');
                } else {
                    $.alert(d.info);
                }
            });
    },
    pushBobbinSwitch: function(className) {
        $(className + ' .tab span').touch(function() {
            var item = $(this).data('item');
            $(this).siblings(className + ' .tab-item').removeClass('on');
            $(this).addClass('on');
            $('.shang_zhuang , .chouma').hide();
            $('.' + $(this).attr('_class')).show();
        });
    },
    douniuGame: function() {
    ...
    submit: function() {
        ...
        if (zhuang_type == 5) {
            data['zhuang_value'] = zhuang_value;
        }
        storage.set('roomrule_niuniu', data);
        if (location.href.split('template=').length > 1) {
            if (location.href.split('template=')[1].split('&')[0] == 1 && win.app > 0) {
                if (typeof Page.mubanInfo == 'function') {
                    Page.mubanInfo(1, '六人牛', data, GameRules);
                    return;
                }
            }
        }
    ...
    douniuTenGame: function() {
        $('.game-list .niuniuTen-bg ').one('click',
            function() {
                ...
                if (data) {
                    $('.niuniuTen-mask .tab span[data-item="' + data.zhuang_type + '"]').touch();
    ...
    douniuTenSubmit: function() {
        ...
        if (location.href.split('template=').length > 1) {
            if (location.href.split('template=')[1].split('&')[0] == 1 && win.app > 0) {
                if (typeof Page.mubanInfo == 'function') {
                    Page.mubanInfo(4, '十人牛', data, GameRules);
                    return;
                }
            }
        }
    ...
    douniuTenSwitch: function(className) {
        $(className + ' .tab span').touch(function() {
            ...
            if (item == 5) {
                ...
                zhuang += '    <div class="name">上庄:</div>';
                zhuang += '    <div class="flex-item">';
                zhuang += '        <label><input type="radio" name="niuniuTen_zhuang_value" value="0"><span>无</span></label>';
                zhuang += '        <label><input type="radio" name="niuniuTen_zhuang_value" value="100"><span>100</span></label>';
                zhuang += '        <label><input type="radio" name="niuniuTen_zhuang_value" value="300"><span>300</span></label>';
                zhuang += '        <label><input type="radio" name="niuniuTen_zhuang_value" value="500" checked><span>500</span></label>';
                zhuang += '   </div>';
                zhuang += '</div>';
                $(className + ' .content > .zhuang').html(zhuang);
            } else {
    ...
    douniuThirteenSubmit: function() {
        ...
        if (location.href.split('template=').length > 1) {
            if (location.href.split('template=')[1].split('&')[0] == 1 && win.app > 0) {
                if (typeof Page.mubanInfo == 'function') {
                    Page.mubanInfo(11, '十三人牛', data, GameRules);
                    return;
                }
            }
        }
    ...
    goldFlowerSubmit: function() {
        ...
        if (location.href.split('template=').length > 1) {
            if (location.href.split('template=')[1].split('&')[0] == 1 && win.app > 0) {
                if (typeof Page.mubanInfo == 'function') {
                    Page.mubanInfo(2, '金花', data, GameRules);
                    return;
                }
            }
        }
    ...
    thirteenCardSubmit: function() {
        ...
        if (location.href.split('template=').length > 1) {
            if (location.href.split('template=')[1].split('&')[0] == 1 && win.app > 0) {
                if (typeof Page.mubanInfo == 'function') {
                    Page.mubanInfo(3, '十三水', data, GameRules);
                    return;
                }
            }
        }
    ...
    threeOpensSubmit: function() {
        ...
        if (location.href.split('template=').length > 1) {
            if (location.href.split('template=')[1].split('&')[0] == 1 && win.app > 0) {
                if (typeof Page.mubanInfo == 'function') {
                    Page.mubanInfo(6, '十人三公', data, GameRules);
                    return;
                }
            }
        }
    ...
    texasPokerSubmit: function() {
        ...
        if (location.href.split('template=').length > 1) {
            if (location.href.split('template=')[1].split('&')[0] == 1 && win.app > 0) {
                if (typeof Page.mubanInfo == 'function') {
                    Page.mubanInfo(5, '德州', data, GameRules);
                    return;
                }
            }
        }
    ...
    fillingPitCardSubmit: function() {
        ...
        if (location.href.split('template=').length > 1) {
            if (location.href.split('template=')[1].split('&')[0] == 1 && win.app > 0) {
                if (typeof Page.mubanInfo == 'function') {
                    Page.mubanInfo(7, '填大坑', data, GameRules);
                    return;
                }
            }
        }
...
Page.template = function() {
    if (location.href.split('template=').length > 1) {
        if (location.href.split('template=')[1].split('&')[0] == 1) {
            $('.index .top').remove();
            $('.index .game-list').css({
                'top': '.2400rem'
            });
            $('.game-list .game-item.thirteenoxen-bg button').css({
                'background': 'url("https://cdn-1255620552.file.myqcloud.com/images/index/app-item01.png") no-repeat 0 0 / 100% 100%',
            });
            $('.game-list .game-item.niuniuTen-bg button').css({
                'background': 'url("https://cdn-1255620552.file.myqcloud.com/images/index/app-item02.png") no-repeat 0 0 / 100% 100%',
            });
            $('.game-list .game-item.niuniu-bg button').css({
                'background': 'url("https://cdn-1255620552.file.myqcloud.com/images/index/app-item03.png") no-repeat 0 0 / 100% 100%',
            });
            $('.game-list .game-item.goldflower-bg button').css({
                'background': 'url("https://cdn-1255620552.file.myqcloud.com/images/index/app-item04.png") no-repeat 0 0 / 100% 100%',
            });
            $('.game-list .game-item.texas-poker-bg button').css({
                'background': 'url("https://cdn-1255620552.file.myqcloud.com/images/index/app-item05.png") no-repeat 0 0 / 100% 100%',
            });
            $('.game-list .game-item.three-opens-bg button').css({
                'background': 'url("https://cdn-1255620552.file.myqcloud.com/images/index/app-item06.png") no-repeat 0 0 / 100% 100%',
            });
            $('.game-list .game-item.thirteencard-bg button').css({
                'background': 'url("https://cdn-1255620552.file.myqcloud.com/images/index/app-item07.png") no-repeat 0 0 / 100% 100%',
            });
            $('.game-list .game-item.fillingPitcard-bg button').css({
                'background': 'url("https://cdn-1255620552.file.myqcloud.com/images/index/app-item08.png") no-repeat 0 0 / 100% 100%',
            });
            $('.createRoomBtn').css({
                'background': 'url("https://cdn-1255620552.file.myqcloud.com/images/index/app-item.png") no-repeat 0 0 / 100% 100%',
            })
        }
    }
};