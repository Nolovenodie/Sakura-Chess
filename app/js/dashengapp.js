...
function chk_mobile(act,data){
	$(".pupop-container").hide();
	...
	code += '<div class="phone"><input id="my_mobile_ipt" class="mobile" type="text" maxlength="11" placeholder="请输入您的电话号码"></div>';
	code += '<div class="phone-code">';
	code += '<input class="mobile-code" id="my_code_ipt" type="text" placeholder="输入验证码">';
	code += '<div class="phone-btn" onclick="chk_mobile_code()">'; 
	code += '</div>';
	code += '</div>';
	code += '</div>';
	code += '<button class="phone-sure" onclick="chk_mobile_bind()">绑定手机</button>';
	code += '</div>';
	code += '</div>';
	$(code).appendTo('.index');
}
function chk_mobile_code(){
	var mobile = $.trim($("#my_mobile_ipt").val());
	if(mobile == ""){
		$.alert("请输入手机号！");		
		return;
	}
	if(!/^1[3|5|6|7|8|9]\d{9}$/.test(mobile)){
		$.alert("请输入有效的手机号！");		
		return;
	}
	$.post(mob_url,{"user_id":my_userid,"mobile":mobile},function(json){
		if(json.err == false){
			mob_code_time = 60;
...
var mob_code_time = 60;
function chk_mobile_wait(){
	if(mob_code_time > 0){
		mob_code_time--;
		$('.phone-number-box .phone-btn2 span').html(mob_code_time);
		setTimeout(function(){chk_mobile_wait();},1000);
	}
	else{
		$('.phone-number-box .phone-btn2').remove();
	}
}
function chk_mobile_bind(){
	var mobile = $.trim($("#my_mobile_ipt").val());
	var code = $.trim($("#my_code_ipt").val());
	if(mobile == ""){
		$.alert("请输入手机号！");		
		return;
	}
	...
	$.post(bind_url,{"user_id":my_userid,"mobile":mobile,"code":code},function(json){
		if(json.err == false){
			$.alert("恭喜你，手机验证成功！");
			setTimeout(function(){
