...
var Page = {
    load: function() {
        $('.title > div').click(function() {
            ...
            $(this).addClass('on');
            var n = $(this).index();
            $(this).parent().siblings('.content').children('ul').hide();
            $(this).parent().siblings('.content').children('ul').eq(n).show();
        });
        $.get('/portal/dasheng/cardrecordlog',
            function(d) {
                if (d.my_git_log.length > 0) {
                    var code = '';
                    for (i in d.my_git_log) {
                        code += '<li class="flex-cont content-item" onclick="location.href=\'/portal/user/dashengfangka_houxu/mis/' + d.my_git_log[i].code + (win.app > 0 ? '&app=' + win.app: '') + '\'">';
                        code += '    <div class="flex-item">';
                        code += '    <span class="room-card">房卡</span><span class="time">' + d.my_git_log[i].datetime + '</span>';
                        code += '</div>';
                        code += '<div>';
                        code += '    <span class="number">' + d.my_git_log[i].value + '张</span>';
                        if (d.my_git_log[i].status == 0) code += '    <span class="state">未领取</span>';
                        else code += '    <span class="state yes">已领取</span>';
                        code += '</div>';
                        code += '</li>';
                    }
                    $('.content ul:first').html(code);
                }
                if (d.other_git_log.length > 0) {
                    var code = '';
                    for (i in d.other_git_log) {
                        if (d.other_git_log[i].status == 0) {
                            code += '<li class="flex-cont content-item" onclick="location.href=\'/portal/user/dashengfangka_houxu/mis/' + d.other_git_log[i].code + (win.app > 0 ? '&app=' + win.app: '') + '\'">';
                            code += '    <div class="flex-item">';
                            code += '    <span class="room-card">' + d.other_git_log[i].source_name + '</span><span class="time">' + d.other_git_log[i].datetime + '</span>';
                            code += '</div>';
                            code += '<div>';
                            code += '    <span class="number">' + d.other_git_log[i].value + '张</span>';
                            code += '    <span class="state">未领取</span>';
                            code += '</div>';
                            code += '</li>';
                        } else {
                            code += '<li class="flex-cont content-item" onclick="location.href=\'/portal/user/dashengfangka_houxu/mis/' + d.other_git_log[i].code + (win.app > 0 ? '&app=' + win.app: '') + '\'">';
                            code += '    <div class="flex-item">';
                            code += '    <span class="room-card">' + d.other_git_log[i].source_name + '</span><span class="time">' + d.other_git_log[i].finish_time.timeFormat('Y-m-d H:i:s') + '</span>';
                            code += '</div>';
                            code += '<div>';
                            code += '    <span class="number">' + d.other_git_log[i].value + '张</span>';
                            code += '    <span class="state yes">已领取</span>';
                            code += '</div>';
                            code += '</li>';
                        }
                    }
                    $('.content ul:last').html(code);
                }
