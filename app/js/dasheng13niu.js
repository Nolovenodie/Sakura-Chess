...
var win = {
    ...
    readyJoin: function(code, func) {
            var d = {"room_users":[],"rule":{"end_points":"1","card_rule":"1","hand_patterns":"","zhuang_value":"0"},"game":"11","max_matches":"12","zhuang_type":"1"};
            //win.gameId = d.game;
            win.gameId = 11;
                ...
                if (d.info == 0) {
                    ...
                    ajax('home/index/result', {
                        ...
                        function(data) {
                            ...
                            if (win.sign == 'xinjia' || win.sign == 'test' || win.sign == 'ceshi') {
                ...
                if (d.first_join || d.first_join == 1) {
                    if (win.version == '1.0.0') {
                        ...
                        if (parseInt(d.game) === 1 || parseInt(d.game) === 4 || parseInt(d.game) === 8 || parseInt(d.game) === 9 || parseInt(d.game) === 10 || parseInt(d.game) === 11) {
                            ...
                            if (parseInt(d.rule.card_rule) === 1) {
                                rule += '二八杠1倍';
                            } else if (parseInt(d.rule.card_rule) === 2) {
                                rule += '二八杠2倍';
                            } else if (parseInt(d.rule.card_rule) === 3) {
                                rule += '二八杠3倍';
                            } else if (parseInt(d.rule.card_rule) === 4) {
                                rule += '二八杠4倍';
                            }
                            if (rule != '') {
                                joinUser += '<p>规则：' + rule + '</p>';
...
var Page = {
    ...
    load: function() {
        ...
        Page.code = "";
        $('.bottom .room-card-number span').html(user.room_cards);
        $('.focus-mask .focus-img img').attr('src', '/images/qrcode-' + win.sign + '.jpg');
        $('.bottom .game-rules').touch(function() {
            $('.rules-mask').show();
        });
        $('.rules-mask').touch(function() {
            $('.rules-mask').hide();
        });
        $('.bottom .game-tips').click(function() {
            AgreementThirteen();
        });
        $('.bottom .game-home').click(function() {
            returnIndexThirteen();
        });
        ...
        /*win.readyJoin(Page.code,
            function() {
                ws.connect('home/douniuThirteen',
                    function() {
...
var game = {
    ...
    init: function(d) {
        ...
        $('.top .top-item span:first').html(d.cur_match);
        $('.top .top-item span:last').html(d.max_matches);
        ...
        if (d.max_matches == 12) {
            var matchesText = '12局×3房卡 ';
        } else if (d.max_matches == 24) {
            var matchesText = '24局×6房卡 ';
        }
        ...
        if (win.app > 0) {
            var index = ['shouquandating', 'dashengzhongyu', 'liuliuxianyue', 'lianyundating'].indexOf(win.sign);
            app_command('share',
                function() {
                    return WEB_TITLE + '：十三人牛' + '\n\r' + '模式:' + typeText + ' 底分:' + d.end_points + '分 规则:' + card_rule + ' 局数:' + matchesText + ' 牌型:' + shareValueText + (d.zhuang_type == 5 ? '上庄:' + d.zhuang_value: '') + '\n\r房间口令：' + ['11', '12', '13', '14'][index] + d.number + '\n\r房间链接：http://' + DOMAIN + '/thirteenoxen.html?code=' + Page.code;
                });
        } else {
            Page.shareData = {
                title: '十三人(房间:' + d.number + ')',
            ...
            for (var i in user_list) {
                ...
                switch (parseInt(d.running)) {
                    case 0:
                        ...
                        $('<div class="banker">').appendTo('.user-info[data-pos="' + bankerpos + '"] .user-img');
                    ...
                    case 3:
                        ...
                        if (user_list[i].is_master == 1) {
                            ...
                            $('<div class="banker">').appendTo('.user-info[data-pos="' + player.pos + '"] .user-img');
                    ...
                    case 4:
                        ...
                        if (user_list[i].code) {
                            ...
                            if (user_list[i].code > 0 && user_list[i].code <= 10) { (function(handCard) {
                                var returnArr = [];
                                for (var i in handCard) {
                                    var arr = [];
                                    arr.push(handCard[i].value);
                                    arr.push(handCard[i].color);
                                    returnArr.push(arr);
                                }
                                for (var a = 0; a < returnArr.length; a++) {
                                    for (var b = a + 1; b < returnArr.length; b++) {
                                        for (var c = b + 1; c < returnArr.length; c++) {
                                            var l = returnArr[a][0];
                                            var m = returnArr[b][0];
                                            var n = returnArr[c][0];
                                            if (l > 10) {
                                            ...
                                            if (parseInt(parseInt(l) + parseInt(m) + parseInt(n)) % 10 === 0) {
                                                var fiveArr = [];
                                                fiveArr.push(returnArr[a]);
                                                fiveArr.push(returnArr[b]);
                                                fiveArr.push(returnArr[c]);
                                                for (var j in returnArr) {
                                                    var bool = false;
                                                    if (returnArr[j].toString() == returnArr[a].toString()) {
                                                        bool = true;
                                                    }
                                                    if (returnArr[j].toString() == returnArr[b].toString()) {
                                                        bool = true;
                                                    }
                                                    if (returnArr[j].toString() == returnArr[c].toString()) {
                                                        bool = true;
                                                    }
                                                    if (!bool) fiveArr.push(returnArr[j]);
                                                }
                                                for (var k in fiveArr) {
                                                    $('.user-card[data-pos="' + player.pos + '"] .cardbox[data-item="' + k + '"] span.card').attr({
                                                        'data-value': fiveArr[k][0],
                                                        'data-color': fiveArr[k][1]
                                                    });
                                                }
                                                return false;
                                            } else {}
                            ...
                            } else if (user_list[i].code == 12) { (function(handCard) {
                                var returnArr = [];
                                for (var i in handCard) {
                                    var arr = [];
                                    arr.push(handCard[i].value);
                                ...
                                for (var a = 0; a < returnArr.length; a++) {
                                    for (var b = a + 1; b < returnArr.length; b++) {
                                        if (returnArr[a][0] != returnArr[b][0]) continue;
                                        for (var c = b + 1; c < returnArr.length; c++) {
                                            if (returnArr[a][0] != returnArr[c][0]) continue;
                                            for (var d = c + 1; d < returnArr.length; d++) {
                                                if (returnArr[a][0] != returnArr[d][0]) continue;
                                                var fiveArr = [returnArr[a], returnArr[b], returnArr[c], returnArr[d]];
                                                fiveArr.sort(function(x, y) {
                                                    return x[1] - y[1];
                                                });
                                                for (var j in returnArr) {
                                                    var bool = false;
                                                    ...
                                                    if (returnArr[j].toString() == returnArr[d].toString()) {
                                                        bool = true;
                                                    }
                                                    if (!bool) fiveArr.push(returnArr[j]);
                                                }
                                                for (var k in fiveArr) {
                                                    $('.user-card[data-pos="' + pos + '"] .cardbox[data-item="' + k + '"] span.card').attr({
                                                        'data-value': fiveArr[k][0],
                                                        'data-color': fiveArr[k][1]
                                                    });
                                                }
    ...
    start: function(data) {
        ...
        $('.top .top-item span:first').html(data.cur_match);
        $('.prepare').hide();
        $('.prepare-item').hide();
        $('.tips-text').remove();
        $('.niuniuNumber').empty();
    ...
    selectedMaster: function(data) {
        ...
        if (!data.random_users) {
            setTimeout(function() {
                    ...
                    $('<div class="banker">').appendTo('.user-info[data-pos="' + pos + '"] .user-img');
                    setTimeout(function() {
    ...
    showdown: function(data) {
        ...
        if (!handCard) {
            ...
            if (data.code > 0 && data.code <= 10) { (function(handCard) {
                var returnArr = [];
                for (var i in handCard) {
                    var arr = [];
                    arr.push(handCard[i].value);
            ...
            } else if (data.code == 12) { (function(handCard) {
                var returnArr = [];
                for (var i in handCard) {
                    var arr = [];
                    arr.push(handCard[i].value);
    ...
    showdownOver: function(data) {
        ...
        for (i in data) {
            var _this = this; (function(i) {
                if (_this.showdown_users.indexOf(data[i].user_id) > -1) return;
                _this.showdown_users.push(data[i].user_id);
                var handCard = data[i].hand_cards;
                var pos = $('.user-info[data-id="' + data[i].user_id + '"]').data('pos');
                if (!handCard) {
                    $.alert('牌型获取失败!', 'error');
                } else {
                    if (data[i].code > 0 && data[i].code <= 10) { (function(handCard, pos) {
                        var returnArr = [];
                        for (var i in handCard) {
                            var arr = [];
                            arr.push(handCard[i].value);
                    ...
                    } else if (data[i].code == 12) { (function(handCard) {
                        console.log(222);
                        var returnArr = [];
                        for (var i in handCard) {
                            var arr = [];
                    ...
                    $('.user-card[data-pos="' + pos + '"] > div').removeClass('hind').addClass('front');
                    if (data[i].code <= 0 || data[i].code > 10) {
                        $('.user-card[data-pos="' + pos + '"]').addClass('nocattle');
                    } else {
                        $('.user-card[data-pos="' + pos + '"]').addClass('cattle');
                    }
    ...
    settlement: function(data) {
        ...
        function getValue() {
            ...
            Gold13(userlost, game.zhuang_id);
        }
        function lostValue() {
            for (var i in data) {
                if (i == 0) continue;
        ...
        function resetGame() {
            ...
            var roomType = $('.zhuangType').attr('data-item');
            if (roomType == 5) {
                if ($('.top .top-item span:first').text() == 3 || $('.top .top-item span:first').text() == 6 || $('.top .top-item span:first').text() == 9) {
                    $('.prepare').hide()
                } else {
                    $('.prepare').show();
                    $('.time').hide();
            ...
            $('.showdown').hide();
            if ($('.top .top-item span:first').text() == $('.top .top-item span:last').text()) {
                $('.tips-text').remove();
                $('.prepare').hide();
                setTimeout(function() {
    ...
    isXiaZhuang: function(data) {
        ...
        function isZhuang() {
            if (data == user.id) {
                var btn = '<div class="zhuang_btn" >';
                btn += '    <div class="upZhuang downZhuang" onclick="game.clickXiaZhuang(1)">下庄</div>';
                btn += '    <div class="upZhuang no downZhuangContinute" onclick="game.clickXiaZhuang(0)")>继续</div>';
                btn += '</div>';
                $(btn).appendTo('.body');
                $('.prepare').hide();
                $('.tips-text').remove();
